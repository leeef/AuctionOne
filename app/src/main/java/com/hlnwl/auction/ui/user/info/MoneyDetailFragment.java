 ...
 * @author yougui
 * 版本：1.0
 * 创建日期：2019/9/29 09:05
 * 描述：
 */
@SuppressLint("ValidFragment")
public class MoneyDetailFragment extends MyLazyFragment implements OnRefreshListener, OnLoadMoreListener {
    ...
    private List<MoneyDetailBean.DataBean> datas=new ArrayList<>();
    @SuppressLint("ValidFragment")
    public MoneyDetailFragment(String type) {
        this.type = type;
    }
    ...
    protected void initView() {
        ...
        if (mAdapter == null) {
            mAdapter = new MoneyDetailAdapter();
    ...
    private void getDatas(String style) {
        RxRetroHttp.composeRequest(RxRetroHttp.create(Api.class)
                .getMoneyDetail(SPUtils.getLanguage(),mPage,
                        SPUtils.getUserId(),SPUtils.getToken(),type), this)
                .subscribe(new ApiObserver<MoneyDetailBean>() {
                               @Override
                               protected void success(MoneyDetailBean data) {
    ...
    private void setData(boolean isRefresh, List<MoneyDetailBean.DataBean> data) {
        showComplete();
        datas.addAll(data);
        mPage++;
