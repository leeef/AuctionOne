...
import com.hlnwl.auction.ui.user.order.CommentActivity;
import com.hlnwl.auction.ui.user.order.OrderAdapter;
import com.hlnwl.auction.ui.user.order.OrderDetailActivity;
...
public class ShopOrderFragment extends MyLazyFragment implements OnRefreshListener, OnLoadMoreListener, BaseQuickAdapter.OnItemChildClickListener, BaseQuickAdapter.OnItemClickListener {
    ...
    private List<ShopOrderBean.DataBean> datas=new ArrayList<>();
    @SuppressLint("ValidFragment")
    public ShopOrderFragment(String type ,String status) {
        this.type = type;
        this.status=status;
    }
    @Override
    ...
    private void getDatas(String style) {
        RxRetroHttp.composeRequest(RxRetroHttp.create(Api.class)
                .getShopOrder(SPUtils.getLanguage(),mPage, SPUtils.getUserId(),SPUtils.getToken(),type,status), this)
                .subscribe(new ApiObserver<ShopOrderBean>() {
                               @Override
                               protected void success(ShopOrderBean
                                                              data) {
    ...
    private void setData(boolean isRefresh, List<ShopOrderBean.DataBean> data) {
        showComplete();
        datas.addAll(data);
        mPage++;
        ...
        switch (view.getId()){
            ...
            case R.id.item_order_qrsh:
                new MessageDialog.Builder(getActivity())
                        .setTitle(StringUtils.getString(R.string.sure_fahuo)) // 标题可以不用填写
                        .setMessage(getActivity().getResources().getString(R.string.sure_fahuo_ask))
    ...
    private void sureOrder(String id) {
        RxRetroHttp.composeRequest(RxRetroHttp.create(Api.class)
                .confirmFh(SPUtils.getLanguage(),SPUtils.getUserId(), SPUtils.getToken(), id), this)
