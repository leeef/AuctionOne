...
public class ShopGoodsFragment extends MyLazyFragment implements BaseQuickAdapter.OnItemClickListener, BaseQuickAdapter.RequestLoadMoreListener {
    ...
    @SuppressLint("ValidFragment")
    public ShopGoodsFragment(String sid) {
        this.sid = sid;
    }
                ...
                .subscribe(new ApiObserver<ShopGoodsBean>() {
                               ...
                               protected void success(ShopGoodsBean data) {
                                   ...
                                   if (style.equals("more")) {
                                       ...
                                       setData(isRefresh, data.getData().get(0).getGlist());
                                   } else if (style.equals("new")) {
                                       datas.clear();
                                       if (data.getData().get(0).getGlist().size() == 0) {
                                           EventBus.getDefault().post(new StopRefreshMessage("refresh"));
//                                           mSrlListCommon.finishLoadMoreWithNoMoreData();
                                           showEmpty();
                                           return;
                                       }
                                       setData(true, data.getData().get(0).getGlist());
                                   }
                               }
